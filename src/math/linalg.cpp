...
Eigen::Matrix3d quaternionToRotation(Eigen::Vector4d& quaternion) {
    Eigen::Matrix3d output;

    double s{2 / (std::pow(quaternion.coeff(0), 2) + std::pow(quaternion.coeff(1), 2) +
                  std::pow(quaternion.coeff(2), 2) + std::pow(quaternion.coeff(3), 2))};
...
Eigen::Vector4d rotationToQuaternion(Eigen::Matrix3d& rotation) {
    Eigen::Vector4d output;

    output.coeffRef(0) =
        std::sqrt(1 + rotation.coeff(0, 0) + rotation.coeff(1, 1) + rotation.coeff(2, 2)) / 2;
    output.coeffRef(1) = sign(rotation.coeff(2, 1) - rotation.coeff(1, 2)) *
                         std::sqrt(1 + rotation.coeff(0, 0) - rotation.coeff(1, 1) - rotation.coeff(2, 2));
    output.coeffRef(2) = sign(rotation.coeff(0, 2) - rotation.coeff(2, 0)) *
                         std::sqrt(1 - rotation.coeff(0, 0) + rotation.coeff(1, 1) - rotation.coeff(2, 2));
    output.coeffRef(3) = sign(rotation.coeff(1, 0) - rotation.coeff(0, 1)) *
                         std::sqrt(1 - rotation.coeff(0, 0) - rotation.coeff(1, 1) + rotation.coeff(2, 2));
...
Eigen::Vector3d transformPoint(Eigen::Vector3d& original_point, Eigen::Vector3d& relative_position,

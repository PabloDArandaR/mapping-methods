...
occupancyGrid::occupancyGrid(int width, int height, int initial_value, double resolution)
...
occupancyGrid::occupancyGrid(std::vector<int8_t>* new_map, int width, int height, int initial_value, double resolution)
...
double occupancyGrid::get_resolution() { return this->resolution; };
...
inline std::tuple<int, int> occupancyGrid::calculate_cell_x_y(double& x, double& y) {
...
inline int occupancyGrid::return_index(int row, int column) { return row * this->width + column; }
